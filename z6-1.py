...
class MST(): 
    ...
    def wyswietl(self):
        ...
        for i in range(len(self.mst)):
            print(' '.join(map(str, self.mst[i])))
            
    def wyswietl_koszt(self):
        koszt = str(self.get_koszt())
        print(f"Łączny koszt: {koszt}")
...
class Listy():
    def __init__(self, krawedzie, n):
        ...
        self.lista_sasiedztwa = self._zrob_liste_sasiedztwa(krawedzie, n)
        self._zrob_liste_wierzcholkow()
        self.lista_grafow = self._zrob_liste_grafow(self.lista_sasiedztwa)
        self._zrob_liste_mostow(krawedzie, n)
        self.lista_komponentow = self._zrob_liste_komponentow(krawedzie, n)
        
    def _zrob_liste_sasiedztwa(self, krawedzie, n):
        lista_sasiedztwa = []
        for i in range(n):
    ...
    def _zrob_liste_mostow(self, krawedzie, n):
        for i in range(len(self.lista_grafow)):
            sciezka_pierwotna = DFS.dfs(self.lista_sasiedztwa, self.lista_grafow[i][0])
            licznik = len(krawedzie)
            for j in range(licznik):
                nowe_krawedzie = krawedzie[:j] + krawedzie[j+1:]
                nowa_lista_sasiedztwa = self._zrob_liste_sasiedztwa(nowe_krawedzie, n)
    ...
    def _zrob_liste_komponentow(self, krawedzie, n):
        ...
        nowa_lista_sasiedztwa = self._zrob_liste_sasiedztwa(n_krawedzie, n)
    ...
    def wyswietl_mosty(self):
        ...
        if len(self.lista_mostow) > 0:
            lista_sort = HeapSort.sortowanie(self.lista_mostow, "M")
            for i in range(len(lista_sort)):
                print(' '.join(map(str, lista_sort[i][:-1])))
        else:
            print("BRAK MOSTÓW")
            
    def wyswietl_komponenty(self):
        print('\nKOMPONENTY:')
        komponent_str = ' '.join(['[' + ' '.join(map(str, komponent)) + ']' for komponent in         self.lista_komponentow])
        print(f"{len(self.lista_komponentow)} KOMPONENTY: {komponent_str}")
...
def main():
    ...
    if bledy_wejscia(wejscie) == True:
        ...
        lstObj = Listy(posortowanaTablica, wejscie[0][0])
        mstObj = MST(posortowanaTablica, wejscie[0][0])
        mst = mstObj.get_mst()  
        mstObj.wyswietl()
        mstObj.wyswietl_koszt()
        lstObj.wyswietl_mosty()
        lstObj.wyswietl_komponenty()
    else:
        print("BŁĄD")
