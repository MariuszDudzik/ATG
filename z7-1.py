...
def zrob_krawedzie(macierz):
    krawedzie = []
    n = len(macierz)
    for i in range(n):
        for j in range(i, len(macierz[i])):
            if macierz[i][j] != 0:
                krawedz = [i+1, j+1, macierz[i][j]]
...
def main():
    ...
    lista_sasiedztwa = zrob_liste_sasiedztwa(macierz)
    dObj = Dijkstra(lista_sasiedztwa, ilosc_w, wierzcholek)
    dObj.wyswietl_odleglosci()
